...
class DynamicScannedLSTM(object):
    ...
    def rnn(self):
        ...
        with tf.name_scope("rnn"):
            ...
            self.outputs, self.final_state = tf.scan(lambda a, x: lstm_cell(x, a[1]),
                                                    tf.transpose(self.rnn_inputs, [1, 0, 2]),
                                                    initializer=(tf.zeros((self.batch_size, self.state_size)),
                                                    self.init_state))
            self.outputs = tf.transpose(self.outputs, [1, 0, 2])
            self.final_state = tuple([tf.nn.rnn_cell.LSTMStateTuple(
                            tf.squeeze(tf.slice(c, [self.num_steps - 1, 0, 0], [1, self.batch_size, self.state_size])),
                            tf.squeeze(tf.slice(h, [self.num_steps - 1, 0, 0], [1, self.batch_size, self.state_size])))
